// Registry authentication, shared by both kinds so they cannot disagree about which host a
// dockerconfigjson credential covers.
package reconciler
...
//
// Matching is on registry host only. Keys were normalised at parse time, because config files
// write the host bare, with a scheme or with a trailing path.
func (k *secretKeychain) Resolve(res authn.Resource) (authn.Authenticator, error) {
	...
	// No credential is not an error: an anonymous pull of a public image may still succeed.
	return authn.Anonymous, nil
}

// KeychainFromSecret builds a keychain from a kubernetes.io/dockerconfigjson Secret.
//
// Only that type is accepted: it is what `kubectl create secret docker-registry` produces and what
// an imagePullSecret already is.
