...
| D8 | A digest a Pod is still running is reclaimed because no live object produces it any more | **NOT mitigated** | D6 refreshes what a live `ImageComposition` or `ImageBuild` references — its current artifact and its `status.history`, capped by `--keep-builds`. A workload pinned to an older digest, or to one whose object has been deleted, is outside that set: nothing pulls it, the registry's window elapses, and the image is gone. **The failure is delayed and displaced** — everything keeps running, because a running Pod does not re-pull; it appears on the next reschedule, node drain or scale-up, as `ErrImagePull` for an image that worked yesterday and a change nobody made today. Raising `--keep-builds` widens the window and does not close it. See ADR 0019, whose premise this supersedes. |
...
| A digest only a Pod still references is not refreshed | D8 | Retention follows live *objects*, not live *workloads*. A pinned digest older than `--keep-builds`, or one whose object was deleted, expires and fails on the next reschedule. ADR 0019 |
