...
| T7 | A cluster member is impersonated and serves or accepts content | **Mitigated, and the unsafe configuration is refused** | Clustered members proxy each other's requests, including authenticated writes, so a rogue member both sees credentials and answers for repositories it owns. Members verify each other against `cluster.tls.cacert`, and the chart **refuses to render** `registry.cluster.enabled` without `registry.tls.enabled` -- the one combination that would put the registry password on the wire on every proxied write. Note the key is `cluster.tls.cacert` and never `http.tls.cacert`: the latter demands a client certificate from every caller including the kubelet, which stops all pulls. |
...
| I9 | The registry's object-storage and cache credentials leak | **Mitigated, with one named exception** | S3 credentials reach zot as environment variables from a Secret (`registry.storage.s3.existingSecret`), never in `config.json` -- a ConfigMap is readable in every `kubectl describe`, the same rule `TestChartCredentialsAreNotFlags` enforces for the composer's own S3. The exception is redis: zot's cache driver accepts only a URL, so a password lives inside `redis://user:pass@host`. For that reason the config is rendered into a **Secret** rather than a ConfigMap whenever clustering is on. |
...
| D9 | A non-persistent redis silently deletes live images | **NOT mitigated, and it cannot be from here** | `extensions.search` records the pull timestamps `keepTags.pulledWithin` depends on -- the mechanism the whole retention guarantee rests on (ADR 0031). Clustering moves that metadata out of per-pod BoltDB and into the shared cache driver, so **a redis restart without persistence loses every timestamp**: every image then looks unpulled, and the next GC pass reclaims images live objects still reference, on a `gcDelay` fuse rather than a retention window. The chart cannot inspect redis, so this is stated in `values.yaml`, NOTES and `docs/registry.md` and owned by the operator. It is D6 arriving through a component this chart does not manage. |
| D10 | A cluster member is down | **Accepted, and named rather than sold** | Clustering **shards**: zot hashes each repository name and exactly one member owns it, so a member that is down makes roughly 1/N of repositories unavailable, and zot's own documentation says the cluster is not self-healing. It buys throughput and survives a rolling update; it does not buy availability. The value is `registry.cluster`, not `registry.ha`, for this reason (ADR 0039). |
...
| A non-persistent redis makes retention delete live images | D9 | Only when clustering is on, and the chart cannot detect it. The sharpest item on this list, because it fails silently and in the deleting direction |
...
| Clustering shards rather than replicates | D10 | A member down takes ~1/N of repositories with it, and zot's cluster is not self-healing |
