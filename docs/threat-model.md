...
| D11 | Attestations expire while the image they describe lives on | **Mitigated** | A referrer manifest has no tag of its subject's, and the shipped policy is `deleteUntagged` with no `keepUntagged` -- so an untagged SBOM or provenance statement would be collected while its subject stayed alive. D6 on a new object type. So every referrer the composer pushes carries its own `digest-` tag, and ones pushed before 0.6.0 are tagged by the backfill along with their subject ([ADR 0060](adr/0060-every-manifest-carries-its-own-name.md)); the refresher lists and pulls each refreshed digest's referrers, which renews those tags. Tests fail when either call is removed. `ImageBuild`'s attestations are children of its image index, and a signature is a `.sig` tag, so `keepTags` covers both. |
