...
| **Release ns → tenant ns** | The builder copies a push credential and the registry CA next to each build | A cluster-wide `create` on Secrets (E6). No longer the only thing written outside this namespace -- see E7 |
...
| I12 | A build's own output reaches status and Events | **Accepted, and bounded** | Since ADR 0046 the kubelet copies the failing container's log tail into `terminationMessage`, and the controller stores it in `status.lastAttempt.message` and echoes it to a Warning Event. That is attacker-influenced content -- whatever a `Dockerfile` prints -- in two new places. Nothing executes it, and it is already readable by anyone who can reach the build pod, so this widens WHERE it appears rather than WHO can see it. Bounded by `MaxLength=4096` and `recon.TruncateTail`, which keeps the end and drops any rune the cut split; an over-long value would otherwise make the status write fail outright, which is why the truncation is not decorative. |
| I11 | A build pod reads another namespace's source | **Closed** | source-controller serves artifacts over plain HTTP with NO authentication, at `/gitrepository/<ns>/<name>/<sha>.tar.gz`, so any pod that can reach it can fetch ANY namespace's source. Build pods used to fetch directly, and the NetworkPolicy operators had to write to make builds work on a default-deny cluster was itself what granted every tenant's build pod that reach. The builder now serves each build its own artifact against a per-build bearer token, and the URL is re-resolved from the `ImageBuild` rather than taken from the request, so a token opens exactly one build and steers nowhere (`internal/buildcontroller/contextproxy.go`, ADR 0044). Build pods no longer need to reach `flux-system` at all. `TestOneBuildsTokenCannotFetchAnothers` is the guard. Residual: with `--context-base-url` unset the old direct fetch remains, which the binary warns about at startup and the chart never does. |
...
| D7 | Refreshing is disabled or misconfigured against a registry that expires content | **Mitigated for the bundled registry; the operator owns it otherwise** | `--retention-refresh-interval=0`, or a window shorter than the interval, silently removes the protection in D6. For the registry the chart installs, both numbers are now rendered by one chart, so it **refuses to install** a margin below 24x or refreshing disabled while a window is set (`templates/_retention.tpl`, `TestChartRefusesARetentionMarginThatIsTooThin`). For a registry you supply the check now runs too: `retention.window` is a DECLARATION of that registry's policy, and the margin is refused against it exactly as for the bundled one. What the chart cannot verify is whether the declaration is true, which is the part the operator owns. |
...
| D11 | Attestations expire while the image they describe lives on | **Mitigated** | An SBOM or provenance statement is a referrer, and has no tag of its subject's. zot, the bundled registry, keeps a referrer for as long as its subject exists, tagged or not (its collector marks every subject-bearing manifest as referenced), so the attestation lives exactly as long as the image -- verified by the upgrade test with `keepUntagged` off. The composer also tags each one after its own digest ([ADR 0060](adr/0060-every-manifest-carries-its-own-name.md)) and the refresher pulls them, for registries that expire untagged content by age instead; zot records no pulls of a referrer, so there the tag lapses harmlessly. `ImageBuild`'s attestations are children of its image index, and a signature is a `.sig` tag. |
...
| E7 | Both controllers write ConfigMaps into any namespace | **Accepted; the controller is the boundary, not RBAC** | `push.writeRefTo` exports a published reference for a consumer to substitute, and both ClusterRoles carry `configmaps: create/update/delete` cluster-wide. That is deliberate: RBAC is granted before an object exists, so permitting an export into whatever namespace its object lives in means permitting it everywhere (ADR 0056). What confines it is code -- `namespaceAllowed` permits the object's own namespace or one named in `--ref-export-namespaces`, and the ConfigMap's name is derived from the object so two objects cannot collide. The residual risk is the largest single grant in the system: **cluster-wide ConfigMap `delete`**, held back only by `ownedBy`, which requires this controller's managed-by label AND both owner labels to match. A bug in either function reaches `flux-system`. No `deletecollection`, so a mistake damages one object at a time. |
